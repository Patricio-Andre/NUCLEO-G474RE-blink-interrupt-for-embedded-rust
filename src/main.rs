...
use stm32g4xx_hal::gpio::Floating;
...
use stm32g4xx_hal::gpio::SignalEdge as SignalEdge;

use stm32g4xx_hal::interrupt;

// Alias for pin button
type ButtonPin = gpioc::PC13<Input<Floating>>;

// Setting Mutex for interupts
static G_BUTTON: Mutex<RefCell<Option<ButtonPin>>> = Mutex::new(RefCell::new(None));
// Setting Mutex for Delay, initial Value 2 seconds
...
fn main() -> ! {
    ...
    // taking core peripherals necessary to SYST
    ...
    unsafe {
        cortex_m::peripheral::NVIC::unmask(stm32g4xx_hal::interrupt::EXTI15_10);
    }
